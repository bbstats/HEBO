...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str,
                        default='/home/asif/workspace/antigenbinding/visualise_results/convergence_curve_config.yaml',
    ...
    for antigen in config['antigens']:
        ...
        for method in config['methods']:
            ...
            if method == 'RealData':
                try:
                    filenames = glob.glob(f"/nfs/aiml/asif/CDRdata/RawBindingsMurine/{antigen}/*.txt")
                ...
                ax.fill_between(list(range(0,f_evals)),
                                 np.mean(binding_energy, axis=0)[:f_evals] - config['num_std'] * np.std(binding_energy, axis=0)[:f_evals],
                                 np.mean(binding_energy, axis=0)[:f_evals] + config['num_std'] * np.std(binding_energy, axis=0)[:f_evals],
    ...
    lgd = fig.legend(handles, labels, bbox_to_anchor=[0.5, y_bbox], loc='upper center', fancybox=True, shadow=True, ncol=n_col_legend, fontsize=15)
    fig.tight_layout(rect=(0., .10, 1, .1))
    plt.savefig(os.path.join(config['save_dir'], "binding_energy_vs_funct_evals.pdf"), bbox_extra_artists=(lgd, ), pad_inches=0.1, bbox_inches='tight')
    plt.close()
