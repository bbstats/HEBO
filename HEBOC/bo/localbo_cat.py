...
class CASMOPOLITANCat:
    ...
    def _create_and_select_candidates(self, X, fX, length, n_training_steps, hypers, return_acq=False):
        ...
        from bo.localbo_utils import local_search, glocal_search
        ...
        def _ei(X, augmented=True):
            ...
            if self.kernel_type in ['rbfBERT','rbf-pca-BERT', 'cosine-BERT', 'cosine-pca-BERT']:
                ...
                # use in-fill criterion
                mu_star = -gp.likelihood(gp(torch.tensor(x_center[0].reshape(1, -1), dtype=torch.float32).to(device))).mean
        ...
        if self.acq in ['ei', 'ucb']:
            if self.batch_size == 1:
                # Sequential setting
                if self.acq == 'ei':
                    X_next, acq_next = search(x_center=x_center[0], f=_ei, config=self.config, max_hamming_dist=length, n_restart=3, batch_size=self.batch_size, cdr_constraints=self.cdr_constraints, seed=self.seed, dtype=self.dtype, device=self.device)
                else:
                    X_next, acq_next = search(x_center=x_center[0], f=_ucb, config=self.config, max_hamming_dist=length, n_restart=3, batch_size=self.batch_size, cdr_constraints=self.cdr_constraints, seed=self.seed, dtype=self.dtype, device=self.device)
            else:
                ...
                for p in range(self.batch_size):
                    if self.acq == 'ei':
                        x_next, acq = search(x_center=x_center[0], f=_ei, config=self.config, max_hamming_dist=length, n_restart=3, batch_size=1, cdr_constraints=self.cdr_constraints, seed=self.seed, dtype=self.dtype, device=self.device)
                    else:
                        x_next, acq = search(x_center=x_center[0], f=_ucb, config=self.config, max_hamming_dist=length, n_restart=3, batch_size=1, cdr_constraints=self.cdr_constraints, seed=self.seed, dtype=self.dtype, device=self.device)
                    ...
                    if self.kernel_type in ['rbfBERT', 'rbf-pca-BERT', 'cosine-BERT', 'cosine-pca-BERT']:
                        ...
                        if self.kernel_type in ['rbf-pca-BERT', 'cosine-pca-BERT']:
                            ...
                            x_next_reprsn = torch.from_numpy(pca.transform(scaler.transform(x_next_reprsn.cpu().numpy())))
                        del bert
