...
class FoldX(BaseTool):
    def __init__(self,
        ...
        config: dictionary of parameters for BO
            antigen: PDB ID of antigen
            chainab: Chain of Antobody
            chainan: Chain of Antigen
            path: path to Absolut installation
            process: Number of CPU processes
            expid: experiment ID
        '''
        for key in ['antigen', 'path', 'process', 'antibody']:
            assert key in config, f"\"{key}\" is not defined in config"
        self.config = config
        assert self.config['startTask'] >= 0 and (self.config['startTask'] + self.config['process'] < os.cpu_count()), \
            f"{self.config['startTask']} is not a valid cpu"
    ...
    def dock(self):
        ...
        current_dir = os.getcwd()
        os.chdir(f"{self.config['path']}")
        subprocess.run(['taskset', '-c', '0-20', './foldx_20221231', '--command=AnalyseComplex', f"--pdb={self.config['antigen']}.pdb",
                             f"--analyseComplexChains={self.config['chainab']},{self.config['chainan']}"], capture_output=True, text=False)

    def Energy(self, x):
        # Step 1: Replace CDRH3 by x in fasta sequence of PDB

        # Step 2: Run AbodyBuilder to get a new PDB structure

        # Step 3: Call dock method. Note dock method requires knowledge of chain of antigen and antibody (available in PDB file)

        # Step 4: Read energy from summary.fxout file
        raise NotImplementedError